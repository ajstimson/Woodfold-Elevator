...
(function($) {
    ...
    $(window).load(function() {
        ...
        function cartMeta() {
            var cartItemID = document.head.querySelector("[property~=cart-item][content]").content;
        ...
        function cartModalLoader(state) {
            var target = $('#cart .modal-body'),
                loader = '<div id="cart-modal-loader" class="load-wrapper"><div class="loader"></div></div>';
        ...
        function cartItemsLoading(status) {
            ...
            if (status === 'on') {
                $('#cart .show-cart tbody').html('<tr id="pre-loader" colspan="10"><td colspan="10"><div class="pulsate-css"></div></td></tr>');
            }
        ...
        function modalHTML() {
            var html = '<div class="modal" id="cart" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">';
        ...
        function displayCart(itemID) {
            ...
            for (var i in cartArray) {
                ...
                output += '</td><td>';
                output += '$<span>' + parseFloat(cartArray[i].price).toFixed(2) + '</span>';
                output += '</td><td>';
                output += cartItemOptions(cartArray[i])
                output += '</td><td id="' + cartArray[i].itemID + '" class="order-item-json" style="display:none">';
        ...
        function itemDetails(json) {
            ...
            html += '</li><li>';
            html += '<button type="button" class="btn btn-primary Click-btnas" data-toggle="modal" data-target="#exampleM">See More...</button>';
            html += '</li></ul>';
            ...
            htmls += '</li><li>Price (USD): $' + parseFloat(json.quote.value).toFixed(2) + '</li></ul>';
        ...
        $('#gform_1').submit(function(event) {
            ...
            wholePageLoader('start');
            var $form = $(this),
                order = formData.cache,
                userID = document.head.querySelector("[property~=user-id][content]").content,
                itemID = document.head.querySelector("[property~=cart-item][content]").content,
                sessionID = document.head.querySelector("[property~=cart-hash][content]").content,
        ...
        $('.clear-cart').click(function() {
            var removeAll = confirm("Are you sure you want to delete the cart's contents?");
            if (removeAll == true) {
                cartModalLoader('start');
                cartLoading();
                clearCart();
        ...
        $('.complete-order').click(function(e) {
            ...
            $(els).each(function() {
                ...
                if (count === i) {
                    ...
                    setTimeout(function() {
                        cartModalLoader('end');
                        window.location.href = "/elevator/user-dashboard?status=success" + query;
                    }, 3000);
        ...
        function duplicateOrder(el) {
            ...
            //Reset item ID
            ajaxCall('GET', 'get_salt', null, null);

            //Get order data
            ajaxCall('POST', 'config_retrieval', null, item_id);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (action === 'insert_order') {
                        ...
                        setTimeout(function() { $("#cart").modal(); }, 3000);
                    ...
                    if (action === 'check_item_id') {
                        ...
                        if (response === '1') {
                            ...
                            document.head.querySelector("[property~=cart-item][content]").content = newHash;
                    ...
                    if (action === 'get_salt') {
                        ...
                        document.head.querySelector("[property~=cart-item][content]").content = cartItem;
                    ...
                    if (action === 'edit_cart_item') {
                        ...
                        if (response === 0) {
                            ...
                            alert('There was a problem retrieving this order. Please try refreshing the page.');
        ...
        function createNewOrder(order) {
            ...
            var newItemID = document.head.querySelector("[property~=cart-item][content]").content;
        ...
        function wholePageLoader(state) {
            var target = $('.site-inner > .wrap'),
                loader = '<div id="whole-page-loader" class="load-wrapper"><div class="loader"></div></div>';
            if (state === 'start') {
                $(target).prepend(loader);
            } else {
