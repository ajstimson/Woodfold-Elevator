(function($) {
    $(window).load(function() {
        ...
        */
        ...
        $(document).on("keyup", '#input_1_13, #input_1_14, #input_1_26, #input_1_24', function() {
            calculateQuote();
        });
    ...
    function quoteElems() {
        ...
        var country = $('#input_1_59'),
            state = $('#input_1_66'),
            zone = $('#input_1_91 input'),
            width = $('#input_1_13'),
            pocketWidth = $('#input_1_26'),
            height = $('#input_1_14'),
            heightOptions = $('#input_1_16'),
            doubleEndGate = $('#input_1_46 input'),
            jambOrientation = $('#input_1_48'),
            panelSwatches = $('#panels-items-right input[type="radio"]'),
            numberOfPanels = $('#input_1_35'),
            panelPosition = $('#input_1_37'),
            visionToggle = $('#input_1_132 input[type="radio"]'),
            visionMaterial = $('#input_1_133'),
            panelFinish = $('#input_1_89'),
            frameSwatches = $('#frames-items-right input[type="radio"]'),
            quantity = $('#input_1_24'),
            rushShipping = $('#choice_1_39_1');
    ...
    function calc() {
        ...
        */
        if (factor.visionPanelSelect === true) {
            ...
            factor.baseQuote = panelCalc(factor.baseQuote, factor.zone, factor.visionPanelCount, visionPrices);
        ...
        */
        ...
        if (factor.acrylic === true) {
            factor.baseQuote = panelCalc(factor.baseQuote, factor.zone, factor.panelCount, price.acrylic);
        }

        if (factor.birchOak === true) {
            factor.baseQuote = panelCalc(factor.baseQuote, factor.zone, factor.panelCount, price.birchOak);
        }

        if (factor.alderMapleMahogany === true) {
            factor.baseQuote = panelCalc(factor.baseQuote, factor.zone, factor.panelCount, price.alderMapleMahogany);
        }

        if (factor.cherryWalnut === true) {
            factor.baseQuote = panelCalc(factor.baseQuote, factor.zone, factor.panelCount, price.cherryWalnut);
        }

        if (factor.vinylWoodgrain === true || factor.vinylSolid === true || factor.mdfNoFinish === true) {
            factor.baseQuote = panelCalc(factor.baseQuote, factor.zone, factor.panelCount, price.vinylWoodgrain);
        }

        if (factor.alumifoldSolid === true) {
            factor.baseQuote = panelCalc(factor.baseQuote, factor.zone, factor.panelCount, price.alumifoldSolid);
        }

        if (factor.perforated === true) {
            factor.baseQuote = panelCalc(factor.baseQuote, factor.zone, factor.panelCount, price.alumifoldPerforated);
        }

        if (factor.fireCore === true) {
            factor.baseQuote = panelCalc(factor.baseQuote, factor.zone, factor.panelCount, price.fireCore);
        }
        ...
        */
        ...
        if (factor.goldUpcharge === true) {
            factor.baseQuote = factor.baseQuote + (factor.baseQuote * price.goldUpcharge);
        }
    ...
    function isChecked(el, arr, vision = false) {
        ...
        if (vision === true) {
            var numberPanels = $('#input_1_35').val(),
                visionPosition = $('#input_1_37').val();
            visionMaterial = $('#input_1_133').val();

            if (numberPanels !== 'false' && visionPosition !== 'false' && visionMaterial !== 'false') {
                checkd = true;
            }
        }
    ...
    function factorArray() {
        ...
        factor.zone = parseInt($('#input_1_91 input:checked').val(), 10);
        factor.height = parseFloat($('#input_1_14').val());
        factor.panelCount = parseInt($('#dim-3 span').text(), 10);
        factor.finish = $('#field_1_89 input:checked').val();
        factor.rush = $('#choice_1_39_1');
        factor.quantity = $('#footer-quantity').val();

        //isChecked parameters are field, array (in the case of hardwood groupings), & vision
        factor.acrylic = isChecked('#field_1_27 input');
        factor.perforated = isChecked('#field_1_131 input');
        factor.birchOakArr = ['Birch', 'Oak'];
        factor.birchOak = isChecked('#field_1_28 input', factor.birchOakArr);
        factor.alderMapleMahoganyArr = ['Alder', 'Maple', 'Mahogany'];
        factor.alderMapleMahogany = isChecked('#field_1_28 input', factor.alderMapleMahoganyArr);
        factor.cherryWalnutArr = ['Cherry', 'Walnut'];
        factor.cherryWalnut = isChecked('#field_1_28 input', factor.cherryWalnutArr);
        factor.mdfNoFinish = isChecked('#choice_1_28_6');
        factor.vinylWoodgrain = isChecked('#field_1_92 input');
        factor.vinylSolid = isChecked('#field_1_101 input');
        factor.alumifoldSolid = isChecked('#field_1_29 input');
        factor.fireCore = isChecked('#field_1_102 input');
        factor.goldUpcharge = isItem('gold');
        factor.doubleGateCharge = isChecked('#field_1_46 input#choice_1_46_0');
        factor.powderCoatCharge = isItem('powder');
        factor.visionPanelSelect = isChecked('#input_1_132 input#choice_1_132_0', undefined, true);
        factor.visionPanelCount = parseFloat($('#input_1_35').val());
        factor.visionMaterial = $('#input_1_133').val();
        factor.baseQuote = 0;
    ...
    function priceArray() {
        ...
        price.acrylic = processPrices($('#input_1_125'), 'array');
        price.birchOak = processPrices($('#input_1_121'), 'array');
        price.alderMapleMahogany = processPrices($('#input_1_127'), 'array');
        price.cherryWalnut = processPrices($('#input_1_126'), 'array');
        price.vinylWoodgrain = processPrices($('#input_1_122'), 'array');
        price.alumifoldSolid = processPrices($('#input_1_123'), 'array');
        price.alumifoldPerforated = processPrices($('#input_1_124'), 'array');
        price.fireCore = processPrices($('#input_1_120'), 'array');
        price.finish = processPrices($('#input_1_119'), 'single');
        price.goldUpcharge = processPrices($('#input_1_129'), 'single');
        price.doubleGateCharge = processPrices($('#input_1_128'), 'single');
        price.rush = processPrices($('#input_1_130'), 'single');
        price.exceeds8 = processPrices($('#input_1_116'), 'single');
        price.powderCoated = processPrices($('#input_1_135'), 'single');
    ...
    function updateQuoteFields(baseQuote) {
        var footerQuote = $('#text-2 .textwidget'),
    ...
    function calcLoader() {
        $('#text-2 .textwidget').html('<div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div>');
    }
