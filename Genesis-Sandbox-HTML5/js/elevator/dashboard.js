(function($) {
    ...
    $(window).load(function() {
        ...
        $(".save-this-config").click(function() {
            var item_id = $(this).data("item"),
                target = document.head.querySelector("[property~=cart-item][content]");
        ...
        $(".clode").click(function() { location.reload(); });
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (action === 'send_email') {
                    ...
                    if (response.client_email === true && response.admin_email === true) {
                        alert('Order Received! You will contacted shortly.');
                        window.history.replaceState(null, null, window.location.pathname);
                    } else {
                        console.log(response);
                        alert('An error occurred! Please refresh this page to resend your order');
                    }
                ...
                if (action === 'create_pdf') {
                    ...
                    if (response !== 'error') {
                        ...
                        var shipToEmpty = $.isEmptyObject(orderData.address_info.customer);
                        ...
                        pdf.setFontSize(22);
                        pdf.text('Gate: Series 1600', addressColumn2, margin.top);
                        pdf.line(margin.left, 21, margin.right, 21);

                        pdf.setFontSize(16);
                        pdf.text('PO Number: ' + orderData.address_info.po_num.po, margin.left, 30);

                        if (orderData.address_info.po_num.sidemark != undefined) {
                            pdf.text('Sidemark: ' + orderData.address_info.po_num.sidemark, margin.left, 40);
                        }

                        pdf.setFontSize(12);
                        pdf.text('Order Date: ' + orderData.address_info.po_num.date, addressColumn2, 30);

                        if (orderData.address_info.po_num.rush != undefined) {
                            pdf.text('Rush Shipping: ' + orderData.address_info.po_num.rush, addressColumn2, 40);
                        }
                        ...
                        pdf.setFont("helvetica", "bold");
                        pdf.setFontSize(16);
                        pdf.text('Configuration Details', margin.left, finalPosition + 20);
                        ...
                        pdf.text('Order Received!', margin.left, margin.bottom - 20);

                        pdf.line(margin.left, margin.bottom - 10, margin.right, margin.bottom - 10);
                        pdf.setFontSize(9);
                        pdf.setTextColor(0, 0, 0);
                        pdf.text('Order Created: ' + orderData.date, margin.left, margin.bottom);
                        pdf.text('Call Woodfold: 503.357.7181', margin.right - 43, margin.bottom);

                        var date = orderData.date.split(' ')[0].replace(/\//g, "");
                        pdf.save('elevator-order-' + date + '-' + orderData.address_info.po_num.po + '.pdf');

                    } else {
                        alert('Order data missing. Please contact the administrator.');
                    }
