(function($) {
    ...
    $(window).load(function() {
        ...
        */
        ...
        //TODO: figure out what setSubdivisions was
    ...
    function defaultAddressData() {
        var arr = [];
        arr.country = $('#input_1_84').val();
        arr.state = $('#input_1_82').val();
    ...
    function selectZoneState(arr) {
        ...
        if (country === 'US' && state === 'CA' || state === 'OR' || state === 'WA') {
            zoneAction(zone1);
        } else

        if (country === 'US' && state !== 'CA' && state !== 'OR' && state !== 'WA' && state !== 'HI' && state !== 'AK') {
            zoneAction(zone2);
        } else
    ...
    function zoneAction(zoneX) {
        ...
        // enable all inputs
        $('#input_1_91 input').attr('disabled', false);
    ...
    function setConfigShipping() {
        var zone = $('#field_1_91 li.selected input').val(),
            config = $('#input_1_38'),
        ...
        if (zone === 3) {
            config.after("<span>Shipping NOT INCLUDED (FOB Origin—call for quote)</span>");
        }
    ...
    function getShipToData() {
        var arr = [];
        arr.country = $('#input_1_59').val();
        arr.state = $('#input_1_66').val();
    ...
    function setQuantity() {
        $('#input_1_24').val(1);
        $('#footer-quantity').val(1);

    }

    function setStackState() {
        var pocket = $('#input_1_26'),
            direct = $('#input_1_20'),
            double = $('#input_1_46'),
            jambOr = $('#input_1_48');
    ...
    function selectDefaultVisionInclude() {
        $('#choice_1_132_1').prop('checked', true);
    }

    function selectDefaultClosure() {
        $('#frames-items-right input[value="Single Magnetic Catch (Standard)"]').prop('checked', true);
    }
    ...
    function renderGate(el = null, defaultFrame = false) {
        ...
        var target = $('#gate-img'),
            vision = visionArray(),
            panel = $('#panels-items-right li.gate-render .gfield_radio input:checked'),
    ...
    function visionArray() {
        var arr = [];
        arr.selected = $('#input_1_132 input:checked').val();
        arr.number = $('#input_1_35').val();
        arr.position = $('#input_1_37').val();
        arr.posSelect = $('#input_1_37 option:checked').text();
        arr.material = $('#input_1_133').val();
    ...
    function setDefaultFrame(el) {
        if (el === null) {
            var panelSelected = $('#panels-items-right .gfield_radio li input:checked')[0].value;
        } else {
        ...
        */
        var frame_arr = ['Anodized Bronze', 'Anodized Bronze', 'Brown', 'Anodized Bronze'];

        if (panelSelected === 'Birch' ||
            panelSelected === 'Maple' ||
            panelSelected === 'Oak' ||
        ...
        if (panelSelected === 'Black' ||
            panelSelected === 'Perforated Black') {
            frame_arr = ['Anodized Silver', 'Anodized Silver', 'Black', 'Black'];
        }

        if (panelSelected === 'Gold' ||
            panelSelected === 'Perforated Gold') {
    ...
    function frameSelections(selection) {
        ...
        $('.hinge-hardware input[value="' + selection[1] + '"]').prop('checked', true),
            $('.lead-post-connector input[value="' + selection[2] + '"]').prop('checked', true);
        $('.side-channels input[value="' + selection[3] + '"]').prop('checked', true);
    ...
    function panelImgArray(panel) {
        ...
        if (panel.val() === 'Acrylic Clear') {
            ...
            arr = [blank, blank, blank, blank, blank, blank, blank, blank, blank, blank];
        ...
        for (var i = 0; i < 14; i++) {
            ...
            if (sweetSpot === true) {
                ...
                if (visionPanels.material === 'Acrylic Clear') {
                    ...
                    var image = $('#panels-items-right input[value="' + visionPanels.material + '"]').data('panel-position-' + panelPosition);
    ...
    function gateAppend(a, t) {
        ...
        var oldGate = document.getElementById('gate-img').getElementsByTagName("img");
        ...
        if (oldGate.length == 0) {
            ...
            for (var i = 0; i < oldGate.length; i++) {
                ...
                if (typeof newGate[i] !== "undefined" && oldGate[i].src !== newGate[i].src) {
                    ...
                    if (newGate[i].classList.contains(targetClass) && !oldGate[i].classList.contains(targetClass)) {
    ...
    function adjustDoorCabHeight() {
        ...
        var height = $('#input_1_14').val(),
            select = $('#input_1_16 option:selected').val(),
    ...
    function autofill(data) {
        ...
        //ADDRESS
        inputFill('#input_1_2', data.po_number.type, data.po_number.value);
        inputFill('#input_1_3', data.sidemark.type, data.sidemark.value);

        if (data.ship_to_country.value.length > 0) {
            //select "Ship to different address"
            $('#input_1_74 input').click();

            inputFill('#input_1_59', data.ship_to_country.type, data.ship_to_country.value);
            inputFill('#input_1_9', data.customer_po_number.type, data.customer_po_number.value);
            inputFill('#input_1_98', data.customer_sidemark.type, data.customer_sidemark.value);
            inputFill('#input_1_63', data.customer_first_name.type, data.customer_first_name.value);
            inputFill('#input_1_64', data.customer_last_name.type, data.customer_last_name.value);
            inputFill('#input_1_87', data.customer_company.type, data.customer_company.value);
            inputFill('#input_1_52', data.ship_to_address.type, data.ship_to_address.value);
            inputFill('#input_1_53', data.apartment_suite_unit_etc.type, data.apartment_suite_unit_etc.value);
            inputFill('#input_1_54', data.ship_to_city.type, data.ship_to_city.value);
            inputFill('#input_1_56', data.state__province__region.type, data.state__province__region.value);
            inputFill('#input_1_66', data.ship_to_state.type, data.ship_to_state.value);
            inputFill('#input_1_67', data.ship_to_region.type, data.ship_to_region.value);
            inputFill('#input_1_68', data.ship_to_province.type, data.ship_to_province.value);
            inputFill('#input_1_57', data.zip__postal_code.type, data.zip__postal_code.value);
            inputFill('#input_1_85', data.customer_phone.type, data.customer_phone.value);
            inputFill('#input_1_86', data.extension.type, data.extension.value);
        ...
        inputFill('#choice_1_39_1', data.rush_shipping.type, data.rush_shipping.value);

        //DIMENSIONS
        inputFill('#input_1_13', data.gate_width.type, data.gate_width.value);
        inputFill('#input_1_14', data.cab_height.type, data.cab_height.value);
        inputFill('#input_1_16', data.height_options.type, data.height_options.value);

        //STACK
        inputFill('#input_1_20', data.stack_direction.type, data.stack_direction.value);
        inputFill('#input_1_26', data.pocket_depth.type, data.pocket_depth.value);
        inputFill('#input_1_46', data.double_ended_gate.type, data.double_ended_gate.value);
        inputFill('#input_1_48', data.change_jamb_orientation.type, data.change_jamb_orientation.value);

        //PANELS
        inputFill('#input_1_27', data.acrylic.type, data.acrylic.value);
        inputFill('#input_1_131', data.alumifold_perforated.type, data.alumifold_perforated.value);
        inputFill('#input_1_29', data.alumifold_solid.type, data.alumifold_solid.value);
        inputFill('#input_1_28', data.natural_hardwood_veneers.type, data.natural_hardwood_veneers.value);
        inputFill('#input_1_92', data.vinyl_laminate_woodgrains.type, data.vinyl_laminate_woodgrains.value);
        inputFill('#input_1_101', data.vinyl_laminate_solid_colors__textures.type, data.vinyl_laminate_solid_colors__textures.value);
        inputFill('#input_1_102', data.fire_core.type, data.fire_core.value);
        inputFill('#input_1_134', data.custom.type, data.custom.value);

        //CUSTOM FINISH SELECTIONS
        inputFill('#input_1_89', data.finish.type, data.finish.value);
        inputFill('#input_1_90', data.enter_type_of_color.type, data.enter_type_of_color.value);

        //VISION PANEL SELECTIONS
        inputFill('#input_1_132', data.include_vision_panels.type, data.include_vision_panels.value);
        inputFill('#input_1_35', data.number_of_vision_panels.type, data.number_of_vision_panels.value);
        inputFill('#input_1_37', data.vision_panel_position.type, data.vision_panel_position.value);
        inputFill('#input_1_133', data.vision_panel_material.type, data.vision_panel_material.value);

        //FRAME
        inputFill('#input_1_88', data.closure_options.type, data.closure_options.value);
        inputFill('#input_1_31', data.lead_post__connector.type, data.lead_post__connector.value);
        inputFill('#input_1_32', data.track.type, data.track.value);
        inputFill('#input_1_33', data.hinge_hardware.type, data.hinge_hardware.value);
        inputFill('#input_1_34', data.side_channels.type, data.side_channels.value);
    ...
    function gateLoader(state) {
        ...
        if (state === 1) {
            $('#gate-img').prepend('<div id="gate-loader" class="lds-ellipsis"><figure><div></div><div></div><div></div><div></div></figure></div>').fadeIn(100);
        }
