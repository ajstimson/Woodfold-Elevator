(function($) {
    $(window).load(function() {
        ...
        */
        ...
        $(document).on('change', '#field_1_89 input', function() {
            var a = $(this);
            toggleColorType(a);
    ...
    function setPositionData() {
        ...
        $('#input_1_37').find('option').each(function() {
    ...
    function additionalClassSet() {
        ...
        //TODO: I think this is unnecessary can can be removed
        $('#input_1_89 li').addClass('no-gate-render');
    }
        ...
        $(s).each(function() {
            ...
            if ($(this).hasClass('panel-item')) {
                ...
                $(items).each(function() {
                    ...
                    if ($item.hasClass('red-oak') || $item.hasClass('natural-oak')) {
    ...
    function setFrameData(el) {
        ...
        var second = getXClass(el, 1),
            actual = second.replace(/^anodized\-/g, '') + res + '.png',
            third = getXClass(el, 2);
    ...
    function toggleColorType(selected) {
        ...
        if (option === 'Special Finish') {
            $('#field_1_90').css('visibility', 'visible')
                .find('input').prop('disabled', false);
        } else {
            $('#field_1_90').css('visibility', 'hidden')
                .find('input').val('').prop('disabled', true);
        }
    }

    function toggleRushShipping(el) {
        var rush = document.getElementById('choice_1_39_1');

        if (el[0].id === 'choice_1_89_2' && rush.checked === true) {
        ...
        if (el[0].id === 'choice_1_89_2') {

            rush.disabled = true;
        }

        if (el[0].id !== 'choice_1_89_2' && document.querySelector('.rush-error') !== null) {
