(function($) {
    $(window).load(function() {
        ...
        window.guest = guestStatus();

        var login = $('#loginform');

        if (login.length === 0) {

            var form = $('#gform_fields_1'),
                footerLi = document.querySelectorAll('#menu-elevator-footer li');

            console.log('ELEVATOR APP VERSION 1.1');
            ...
            if (window.guest === true) {
                kickTheStruts();
                var dimensions = document.querySelector('#menu-item-1690 a');
    ...
    function entrySections(form) {
        var html = '<li id="main-display" class="entry-section flex-66">' +
            '</li>' +
    ...
    function wrapSections() {
        var addressL = $('.address-item.left'),
            ...
            stack = $('.stack-item'),
            stackCollapse = $('#field_1_44, #field_1_43'),
            panel = $('.panel-item'),
            vision = $('#field_1_35, #field_1_37, #field_1_133'),
            frame = $('.frame-item'),
            reviewL = $('#field_1_72'),
            reviewR = $('.review-right, .gform_footer');

        $(addressL).wrapAll('<ul id="address-items-left" class="review-active address-content flex-item" data-content="address"/>');
        $(addressR).wrapAll('<ul id="address-items-right" class="review-active address-content flex-item"  data-content="address" />');
        $(dimensionsL).wrapAll('<ul id="dimension-items-left" class="review-active dimensions-content flex-item" />');
        $(dimensionsR).wrapAll('<ul id="dimension-items-right" class="review-active dimensions-content flex-item input-v2"  data-content="dimensions"/>');
        $(stack).wrapAll('<ul id="stack-items-right" class="review-active stack-content flex-item input-v2"  data-content="stack"/>');
        $(stackCollapse).wrapAll('<li id="stack-items-collapse" class="flex-row"/>');
        $(panel).wrapAll('<ul id="panels-items-right" class="review-active panels-content flex-item input-v2"  data-content="panels"/>');
        $(vision).wrapAll('<ul id="vision-selections" class="flex-item">');
        $(frame).wrapAll('<ul id="frames-items-right" class="review-active frames-content flex-item input-v2"  data-content="frames"/>');
        $(reviewL).wrap('<ul id="review-items-left" class="review-active review-content flex-item" data-content="review" />');
        $(reviewR).wrapAll('<ul id="review-items-right" class="review-active review-content flex-item" data-content="review" />');
    ...
    function reviewSection() {
        var target = $('#review-items-left'),
            save = '<p>&#8210; OR &#8210;</p><div id="form-save"><button>Save For Later</button></div>',
            submit = $('.gform_footer');
        ...
        if (window.guest === true) {
            $('#field_1_100').after('<div id="request-quote"><button class="disabled">Submit Request</button></div>');
        }

    }

    function heightOptionsData() {
        var option1 = $('#input_1_16 option[value=".3125"]'),
            option2 = $('#input_1_16 option[value=".4375"]'),
            option3 = $('#input_1_16 option[value=".8125"]');

        $(option1).attr('data-increase', .5);
        $(option2).attr('data-increase', .375);
    ...
    function setJambNote() {
        var target = $('#field_1_13 .ginput_container'),
            message = '<div id="width-jamb-status" class="instruction">Jamb Orientation: <span></span></div>';
        $(target).append(message);
    }

    function quantityField() {
        var target = $('#text-3 .textwidget'),
            field = '<input id="footer-quantity" type="number" class="check-limits trigger-quote">';
    ...
    function setTooltips() {
        $('.footer-wrap').before('<div id="required-po-tooltip" class="el-tooltip"><span>PO Number Required</span></div>');
        $('.footer-wrap').before('<div id="dimensions-tooltip" data="dimensions" class="el-tooltip dim-stack"><span> Error</span></div>');
        $('.footer-wrap').before('<div id="stack-tooltip" data="stack" class="el-tooltip dim-stack"><span>Error</span></div>');
        $('.footer-wrap').before('<div id="quantity-tooltip" data="quantity" class="el-tooltip"><span>Quantity Required</span></div>');
        $('.footer-wrap').before('<div id="vision-panel-number-tooltip" data="vision-panel" class="el-tooltip"><span>Select Number of Vision Panels</span></div>');
    }
    ...
    function setCalcSectionClasses() {
        ...
        $('#menu-elevator-footer .dimensions-content, ' +
            ...
            'div[data-direction="frames-content"]').addClass(className);
    ...
    function emphasizeQuantityField(clicked) {
        var quantContainer = $('#text-3 .textwidget');
    ...
    function directionalButtons() {
        ...
        $(sections).each(function(i) {
            ...
            var previous = contentArr[i - 1],
                next = contentArr[i + 1],
                svg = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 32 32"><path d="M12.969 4.281l11 11 .688.719-.688.719-11 11-1.438-1.438L21.812 16 11.531 5.719z"></path></svg>';
            ...
            if (previous !== undefined) {
                ...
                html += '<div class="previous reform" data-direction ="' + previous + '-content">';
            ...
            if (next !== undefined) {
                ...
                html += '<div class="next reform" data-direction="' + next + '-content">'
    ...
    function kickTheStruts() {
        ...
        address.footer = document.getElementById('menu-item-1689');
